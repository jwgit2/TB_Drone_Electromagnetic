...
for file in glob.glob("*.csv"):
    with open(file) as inp:
        ...
        #print(timestamp)
        height = np.array([float(numeric_string) for numeric_string in currentData[11]])
        plt.plot( timestamp,height, color='b', label='Height [m]')
        #plt.xscale('log')
        ...
        #plt.show()
        plt.savefig(os.path.splitext(file)[0]+"_height.png", bbox_inches='tight')
        plt.close()
