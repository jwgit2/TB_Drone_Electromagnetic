...
for file in glob.glob("*.csv"):
    with open(file) as inp:
        ...
        #print(timestamp)
        height = np.array([float(numeric_string) for numeric_string in currentData[10]])
        plt.plot( timestamp,height, color='b', label='East in decimal degrees')
        #plt.xscale('log')
        ...
        #plt.show()
        plt.savefig(os.path.splitext(file)[0]+"_east.png", bbox_inches='tight')
        plt.close()
